...
- expand `just ci` with `just --dry-run ci` when that target exists
- inspect delegated scripts and package commands reached by the expanded recipe
- map expensive invocations to distinct proof dimensions and detect exact, transitive, or semantic repetition
...
- ensure one expanded `just ci` run executes each expensive proof dimension once unless a repeated invocation materially changes scope, mode, inputs, environment, artifact, isolation boundary, or failure mode
- do not compose `ci` from both a composite target and the same work reached transitively through that target
...
- re-run `just --dry-run ci`, inspect delegated wrappers, and account for every distinct expensive proof before running the full gate
