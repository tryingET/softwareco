...
# Profiles:
#   - local-dev   : resolve refs by default for this ontology overlay repo
...
case "$ROCS_CI_PROFILE" in
  local-dev)
    # This repo is a company overlay with required ref-layer dependencies,
    # so local validation/build should use the canonical resolved mode too.
    strict_gate
    ;;
