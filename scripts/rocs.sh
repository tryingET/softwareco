...
owner_repo="$(cd -- "$script_dir/.." && pwd -P)"
repo="${ROCS_REPO:-$owner_repo}"
if [[ "$repo" != "$owner_repo" ]]; then
  echo "error: ROCS_REPO must be the Softwareco parent: $owner_repo" >&2
  exit 2
fi
repo="$owner_repo"
...
for ((index = 0; index < ${#arguments[@]}; index++)); do
  ...
  case "${arguments[index]}" in
    --repo)
      ((index + 1 < ${#arguments[@]})) || {
        echo "error: --repo requires the Softwareco parent path" >&2
        exit 2
      }
      ...
      ;;
    --repo=*)
      candidate="${arguments[index]#--repo=}"
      ;;
  esac
  if [[ -n "$candidate" ]]; then
    candidate="$(cd -- "$candidate" 2>/dev/null && pwd -P)" || {
      echo "error: --repo is not a readable directory: $candidate" >&2
      exit 2
    }
    if [[ "$candidate" != "$owner_repo" ]]; then
      echo "error: --repo must be the Softwareco parent: $owner_repo" >&2
      exit 2
    fi
  fi
done
...
case "${1:-}" in
  --which)
    printf 'owner-generated sealed vendored ROCS 0.4.2: %s\n' "$artifact"
    exit 0
    ;;
  --doctor)
    set -- vendored-check --vendored-dir "$artifact"
    ;;
esac
...
_TRUSTED_RECEIPT_SHA256 = "259e5264e9c3dc620981448a75efbad0d840d213898c9cacf87655b3980d80b7"
...
def _main() -> int:
    ...
    try:
        ...
        return _run_captured_argv(sys.argv[3:])
    finally:
